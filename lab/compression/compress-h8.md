...
✗ Agents only consult skills for tasks requiring capabilities beyond what they can handle alone.
— Dropped "typically" (tendency → absolute rule). Collapsed "knowledge or capabilities" → "capabilities" (lost a dimension).
✓ Agents typically only consult skills for tasks requiring knowledge or capabilities beyond what they can handle alone.
...
✗ Content the LLM can already access increases reasoning tokens 10–22% without improving outcomes.
— Collapsed "already knows" (parametric) and "can discover" (retrieval) into "can already access." Two distinct mechanisms merged. Dropped source list.
✓ Restating content the target LLM already knows or can discover from existing sources (docs, code, READMEs) increases reasoning tokens 10–22% without improving outcomes.